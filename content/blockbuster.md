...
Also, you need [Aperture](<?php echo $links['aperture']['curse'] ?>)'s and [Metamorph](<?php echo $links['metamorph']['curse'] ?>)'s dev builds. Build them and then grab `-sources.jar` and `-dev.jar` to `run/libs/` folder, and refresh the IDE.
<?php endif ?>